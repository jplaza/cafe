...
; (defn update [:put "/api/users/:id"] {:keys [user id]}
;   (if (users/update-attributes (assoc user :id id))
;     (resp/json {:message "User updated"})
;     (resp/status 404 (resp/json {:message "User not found"}))))

; (defn destroy [:delete "/api/users/:id"] {:keys [id]}
;   (if (users/destroy id)
;     (resp/json {:message "User deleted"})
;     (resp/status 404 (resp/json {:message "User not found"}))))

; (defn session-create [:post "/api/session/"] {:keys [email password]}
